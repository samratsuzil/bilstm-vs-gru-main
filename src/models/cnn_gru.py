...
class CNNGRU(nn.Module):
    def __init__(self, num_classes, dropout=0.5, use_resnet50=False, hidden_size=512, num_layers=3):
        """
        Enhanced GRU model for 85%+ accuracy
        Args:
            num_classes: Number of output classes
            dropout: Dropout rate
            use_resnet50: Use ResNet50 instead of ResNet18
            hidden_size: GRU hidden size (default 512 for more capacity)
            num_layers: Number of GRU layers (default 3 for deeper model)
        """
        super().__init__()
        self.cnn = CNNBackbone(use_resnet50=use_resnet50)
        cnn_output_dim = self.cnn.output_dim

        # Powerful GRU with more layers and larger hidden size
        self.rnn = nn.GRU(
        ...
        # Deeper classifier with more capacity
        self.dropout = nn.Dropout(dropout)
        self.fc1 = nn.Linear(rnn_output_dim, 512)
